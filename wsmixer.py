...
async def mouseconnect():
    global token
    try:    
        req = requests.post('https://xixya.com/api/log', json = {"username":USERNAME,"password":PASSWORD},timeout=8)
    ...
    token = req["token"]
    req = requests.post('https://xixya.com/api/mixerhotkeys', json = {"token":token,"hotkeys":hotkeys},timeout=8)
    req = req.json()
    if req["message"]!="ok":
        print(req["message"])
    else:
...
def setvolume(volume):
    ...
    for i in range (len(levels)):
        convertedlevels[list(levels.keys())[i]+".exe"] = levels[list(levels.keys())[i]]
    ...
    while True:
        ...
        for app in w:
            if app.Process.name() not in list(colors.keys()): 
                colors[app.Process.name()] = colorgetter.get_most_common_color(app.Process.exe())
            
            if colors[app.Process.name()] != (-2,-1,-1):
                apps[app.Process.name().split(".exe")[0]] = [round(app.SimpleAudioVolume.GetMasterVolume(),2),colors[app.Process.name()]]
            names.append(app.Process.name())
        try:
        
            r = requests.post('https://xixya.com/api/mixerargs', json = {"apps":apps,"token":token,"type":"endclient"},timeout=6)
